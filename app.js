...
var client = io.sockets.on('connection', function(client) {
  client.emit('hello', {msg: 'SOCKET.IO: EU MANDO VALORES'});
...
//Intervalo de 1 em 1 segundo para pegar os valores em tempo real dos sensores
setInterval(function() {
request.get("http://177.16.15.134/api/OPCUA/Sensor/TT6205/value", (error, response, body) => {
  if(error){
    console.log(error);
  }
  ...
  client.emit('valores', {Value: sensor.Value, Status: sensor.Status, Data: date });
});
}, 1000);
...
//Cria o servidor TCP/IP e passa um evento como parametro para toda vez que um cliente
// se conectar
/*
